...
class Node:
    ...
    def __lt__(self, node):
        return self.path_cost < node.path_cost  # self.state < node.state
...
def depth_first_tree_search_modifiedLucio(problem):
    """
    [Figure 3.7]
    Search the deepest nodes in the search tree first.
    Search through the successors of a problem to find a goal.
    ...
    while frontier:
        ...
        if problem.goal_test(node.state):
            return node
        reachables = node.expand(problem)
        frontier.extend(
            [r for r in reachables if r.state not in visited and r not in frontier]
        )
    return None
...
def compare_searchers_of(
    ...
    table = [[name(s)] + [s(problems[0]).solution()] for s in searchers]
    print_table(table, header)
...
class CanibalMisioneros(Problem):
    def __init__(self, initial, goal=(0, 0, 3, 3)):
        super().__init__(initial, goal)

    def goal_test(self, state):
        return state == self.goal  # 3 canibales y 3 misioneros en la orilla derecha.

    def actions(self, state):
        posible_actions = ["MM", "CC", "MC", "M", "C"]
        problem_initial_state = (3, 3, 0, 0)
        # 0 = misioneros izquierda
        # 1 = canibales izquierda
        # 2 = misioneros derecha
        # 3 = canibales derecha
        ML = 0  # misioneros
        CL = 1  # canibales
        MR = 2  # misioneros
        CR = 3  # canibales
        if state == problem_initial_state:
            return posible_actions
        elif state[CL] > state[ML] or state[CR] > state[MR]:
            return []
        elif state[CL] == 1 and state[ML] > 1:
            posible_actions.remove("CC")
        elif state[ML] == 1 and state[CL] == 1:
            posible_actions.remove("MM")
            posible_actions.remove("CC")
    ...
    def result(self, state, action):
        ...
        if action == "MM":
            ...
            state_temp[CL] -= 1
            state_temp[CR] += 1
        return tuple(state_temp)


class NPuzzles(Problem):
    def __init__(self, initial, n, goal=None):
        self.initial = initial
        self.sqrt_n = int(sqrt(n + 1))
        self.goal = goal
        self.n = n + 1
    ...
    def goal_test(self, state):
        # Quedarse con los primeros n-1 elementos de la tupla y
        # verificar que esten ordenados de menor a mayor
        list_n1 = list(state[:-1])
        return list_n1 == sorted(list_n1) and state[-1] == 0

    def actions(self, state):
        possible_actions = ["up", "down", "left", "right"]
        index_blank_square = self.find_blank_square(state)

        if index_blank_square % self.sqrt_n == 0:
            possible_actions.remove("left")
        if index_blank_square < self.sqrt_n:
            possible_actions.remove("up")
        if index_blank_square % self.sqrt_n == self.sqrt_n - 1:
            possible_actions.remove("right")
        if index_blank_square >= self.n - self.sqrt_n:
            possible_actions.remove("down")
        return possible_actions

    def result(self, state, action):
        index_blank = self.find_blank_square(state)
        new_state = list(state)
        delta = {"up": -self.sqrt_n, "down": self.sqrt_n, "left": -1, "right": 1}
        self.swap(new_state, index_blank, index_blank + delta[action])
        return tuple(new_state)

    def swap(self, list_state, i, j):
        element = list_state[i]
        list_state[i] = list_state[j]
        list_state[j] = element
